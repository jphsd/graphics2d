...
func DrawPoint(img *image.RGBA, at []float64, color color.RGBA) {
	DrawPointW(img, at, 1, color)
}

// DrawPointW renders a point of size width into img.
func DrawPointW(img *image.RGBA, at []float64, width float64, color color.RGBA) {
...
func DrawLine(img *image.RGBA, start, end []float64, color color.RGBA) {
	DrawLineW(img, start, end, 1, color)
}

// DrawLineW renders a line of size width into img.
func DrawLineW(img *image.RGBA, start, end []float64, width float64, color color.RGBA) {
	RenderColoredShape(img, NewShape(
...
func DrawArc(img *image.RGBA, start, center []float64, radians float64, color color.RGBA) {
	DrawArcW(img, start, center, radians, 1, color)
}

// DrawArcW renders an arc of size width into img.
func DrawArcW(img *image.RGBA, start, center []float64, radians float64, width float64, color color.RGBA) {
	RenderColoredShape(img, NewShape(
...
func DrawPath(img *image.RGBA, path *Path, color color.RGBA) {
	DrawPathW(img, path, 1, color)
}

// DrawPathW renders a path of size width into img.
func DrawPathW(img *image.RGBA, path *Path, width float64, color color.RGBA) {
	RenderColoredShape(img, NewShape(
